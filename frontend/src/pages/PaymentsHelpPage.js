...
export default function PaymentsHelpPage() {
  return (
    <div className="min-h-screen bg-stone-50">
      {/* Hero */}
      <section className="relative py-16 sm:py-20 px-4 bg-gradient-to-br from-stone-900 to-brand-950 text-white">
          ...
          <nav className="flex flex-wrap gap-2" aria-label="Payment topics">
            {PAYMENTS_TOPICS.map((t) => (
              <a
                key={t.id}
                href={`#${t.id}`}
                className="inline-flex items-center gap-1 rounded-full border border-stone-200 px-3 py-1.5 text-sm text-stone-700 hover:border-brand-300 hover:text-brand-800 transition-colors"
              >
                {t.title}
      ...
      <section className="py-16 px-4 bg-stone-900 text-center">
        <Headset size={36} weight="duotone" className="text-brand-300 mx-auto mb-4" />
        ...
        <div className="flex flex-col sm:flex-row items-center justify-center gap-3">
          <a
            ...
            className="inline-flex items-center gap-2 px-6 py-3 bg-white text-stone-900 rounded-full font-bold hover:bg-brand-50 transition-colors"
          >
            Email {SUPPORT_EMAIL}
          </a>
          <a
            href={`tel:${SUPPORT_PHONE}`}
            className="inline-flex items-center gap-2 px-6 py-3 border border-stone-600 text-stone-200 rounded-full font-bold hover:border-brand-300 hover:text-brand-300 transition-colors"
          >
        ...
        <p className="mt-6 text-stone-500 text-sm">
          Salon owner?{' '}
          <Link to="/for-salons" className="text-brand-300 hover:underline">
