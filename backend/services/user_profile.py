...
async def resolve_profile_for_user(
    ...
    *,
    ...
    Note: this function is used for EXISTING user login paths only.
    New user profile creation goes through create_new_profile().
    """
    ...
    logger.warning(
        "resolve_profile_for_user: creating repair profile user=%s role=%s",
        user_id[:8] if user_id else "?",
        fallback["role"],
    )
