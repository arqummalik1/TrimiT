-- Migration 56: Harden reschedule_booking_atomic with auth.uid() + employee role
-- Forward-only. Apply manually in Supabase SQL Editor.

-- Allow 'employee' in reschedule audit trail
ALTER TABLE public.booking_reschedules
  DROP CONSTRAINT IF EXISTS booking_reschedules_initiated_by_role_check;

ALTER TABLE public.booking_reschedules
  ADD CONSTRAINT booking_reschedules_initiated_by_role_check
  CHECK (initiated_by_role IN ('customer', 'owner', 'employee'));
...
BEGIN
  -- Auth hardening: caller must match JWT subject
  ...
  IF p_user_role = 'employee' THEN
    IF NOT EXISTS (
      ...
      WHERE st.salon_id = v_booking.salon_id
        AND st.user_id = p_user_id
        AND st.app_access_status = 'active'
  ...
  IF v_booking.staff_id IS NOT NULL THEN
    SELECT duration INTO v_service_duration FROM services WHERE id = v_booking.service_id;
    IF v_service_duration IS NULL THEN v_service_duration := 30; END IF;

    SELECT is_active, working_hours INTO v_staff_active, v_staff_working_hours
    FROM staff WHERE id = v_booking.staff_id;
    ...
    v_day_of_week := TRIM(LOWER(TO_CHAR(p_new_date, 'Day')));
    ...
    SELECT p_new_date::TEXT = ANY(days_off) INTO v_is_day_off FROM staff WHERE id = v_booking.staff_id;
    ...
    SELECT COUNT(*) INTO v_staff_conflicts
    FROM bookings b
  ...
  SELECT s.max_bookings_per_slot, s.allow_multiple_bookings_per_slot
  INTO v_max_bookings, v_allow_multiple
  FROM salons s WHERE s.id = v_booking.salon_id FOR UPDATE;
  ...
  INSERT INTO booking_reschedules (
    booking_id, old_date, old_time_slot, new_date, new_time_slot,
    initiated_by, initiated_by_role, reason
  ) VALUES (
    p_booking_id, v_old_date, v_old_time_slot, p_new_date, v_norm_slot,
    p_user_id, p_user_role, p_reason
  );
...
SELECT 'Migration 56 applied: reschedule_booking_atomic auth.uid() + employee role' AS status;
